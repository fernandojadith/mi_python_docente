...
class Formacion(models.Model):
    ...
    MOMENTO_CHOICES = [
        ...
        ("intersemestral_1", "Intersemestral 1"),
        ("intersemestral_2", "Intersemestral 2"),
    ]
    ...
    momento = models.CharField(
        max_length=25,  # Incrementado para acomodar "intersemestral_1" y "intersemestral_2"
    ...
    @property
    def momento_display(self):
        """
        Retorna el momento de manera más legible para mostrar en templates.
        """
        momento_dict = {
            'semestre_1': 'Semestre 1',
            'semestre_2': 'Semestre 2',
        ...
        return momento_dict.get(self.momento, self.momento)
...
class FormacionDocente(models.Model):
    ...
    def efectividad(self):
        if (
            ...
            promedio = (
                self.evaluacion_de_conocimiento +
                self.nivel_de_satisfaccion +
                self.desempeño_laboral
            ) / 3
    ...
    def save(self, *args, **kwargs):
        """
        Sobrescribimos save para que id_programa SIEMPRE coincida con el id_programa del usuario.
        """
        if self.id_usuario and self.id_usuario.id_programa:
            if self.id_programa and self.id_programa != self.id_usuario.id_programa:
                # ⚠️ Inconsistencia detectada
                print(f"⚠️ Inconsistencia detectada: Usuario {self.id_usuario} tiene programa "
                      f"{self.id_usuario.id_programa}, pero se intentó asignar {self.id_programa}. "
                      f"Se corrigió automáticamente.")
            # Se fuerza a que siempre sea el del usuario
            self.id_programa = self.id_usuario.id_programa
